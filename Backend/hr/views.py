...
def get_monthly_calendar_data(user, year, month):
    import calendar
    from datetime import date
    from hr.models import Attendance, HRSettings, recalculate_attendance_for_user_month
    
    settings = HRSettings.get_settings()
    first_weekday, num_days = calendar.monthrange(year, month)
    
    # Always recalculate to guarantee fresh alignment
    recalculate_attendance_for_user_month(user, year, month)
    ...
    for r in records:
        ...
        if r.is_weekoff:
            ...
            stats['holiday'] += 1
        elif r.is_sandwich_leave:
            stats['sandwich_leave'] += 1
            ...
            stats['half_day'] += 1
            stats['total_working_days'] += 1
        elif 'wfh' in status_lower or 'work from home' in status_lower:
            stats['wfh'] += 1
            stats['total_working_days'] += 1
        ...
        if r.is_weekoff:
            ...
            color_class = 'calendar-sandwich-leave'
        elif 'present' in status_lower:
        ...
        working_hours = r.total_hours or 0
        overtime = 0
        if working_hours > 9:
        ...
        days_data.append({
            ...
            'date_formatted': r.date.strftime('%A, %b %d, %Y'),
            'status': status,
            'original_status': r.original_status or '',
            'calculated_status': r.calculated_status or '',
            'is_weekoff': r.is_weekoff,
            'is_holiday': r.is_holiday,
            'is_sandwich_leave': r.is_sandwich_leave,
...
def calculate_payroll_data(employee, year, month):
    ...
    recalculate_attendance_for_user_month(employee, year, month)

    records = Attendance.objects.filter(user=employee, date__year=year, date__month=month)
    ...
    for r in records:
        ...
        if r.is_sandwich_leave:
            ...
            holidays_count += 1
        elif 'present' in status_lower or status == 'Work From Home' or status == 'On Duty':
            present_days += 1
        elif 'half' in status_lower:
            half_days += 1
        elif 'leave' in status_lower:
            from hr.models import Leave
            lv = Leave.objects.filter(user=employee, status__in=['Approved', 'Final Approved'], from_date__lte=r.date, to_date__gte=r.date).first()
            if lv and lv.leave_type == 'Unpaid':
                unpaid_leave_days += 1
            else:
                paid_leave_days += 1
        elif 'absent' in status_lower or status == 'Future':
            absent_days += 1
