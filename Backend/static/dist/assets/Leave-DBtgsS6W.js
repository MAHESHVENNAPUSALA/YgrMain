import{c as e,d as t,f as n,l as r,n as ee,o as i,r as te,t as ne,u as a}from"./index-OYoV3RNy.js";var o=n(a(),1),s=t(),c=()=>{let{user:t}=te(),{showToast:n}=ee(),{prompt:a}=ne(),c=t?.role,re=e(),l=r(),u=re.state,d=window.location.pathname,f=d.includes(`leave-status`)||d.includes(`apply-leave`)||!d.includes(`approved-leaves`)&&!d.includes(`leave-requests`)&&!d.includes(`all-leaves`)&&!d.includes(`leave-dashboard`),p=d.includes(`approved-leaves`)||d.includes(`tl-approved-leaves`)||d.includes(`hr-approved-leaves`)||d.includes(`manager-approved-leaves`),m=d.includes(`leave-requests`)||d.includes(`leave-dashboard`),h=d.includes(`all-leaves`),[g,_]=(0,o.useState)(d.includes(`apply-leave`)?`apply`:f?`list`:`approvals`),[v,ie]=(0,o.useState)([]),[ae,oe]=(0,o.useState)(24),[y,se]=(0,o.useState)(0),[b,x]=(0,o.useState)(!0),[ce,le]=(0,o.useState)(null),[S,ue]=(0,o.useState)(null),[de,C]=(0,o.useState)(!1),[w,fe]=(0,o.useState)(`created_at`),[T,E]=(0,o.useState)(`desc`),[D,O]=(0,o.useState)(1),[k,A]=(0,o.useState)(``),[j,M]=(0,o.useState)(``),[N,pe]=(0,o.useState)(`Paid`),[P,F]=(0,o.useState)(``),[I,L]=(0,o.useState)(!1),[R,z]=(0,o.useState)(``),[B,V]=(0,o.useState)(u?.statusFilter||``);(0,o.useEffect)(()=>{u&&u.statusFilter&&V(u.statusFilter)},[u]);let H=async()=>{x(!0);try{let e=`personal`;p?e=`approved-tracking`:h?e=`team-all`:m&&(e=`team-pending`);let t={scope:e};B&&(t.status=B);let n=await i.get(`/api/leaves/`,{params:t});ie(n.data.leaves||[]),oe(n.data.leave_balance??24),se(n.data.approved_count??0),le(n.data),O(1)}catch(e){console.error(`Error loading leaves:`,e)}finally{x(!1)}};(0,o.useEffect)(()=>{V(``),z(``),O(1),H()},[d]),(0,o.useEffect)(()=>{H()},[B]);let me=async e=>{e.preventDefault(),L(!0);try{await i.post(`/api/leaves/`,{from_date:k,to_date:j,leave_type:N,reason:P}),n(`Leave application submitted successfully.`,`success`),A(``),M(``),F(``),d.includes(`apply-leave`)?l(`/leave-status`):(_(`list`),H())}catch(e){n(e.response?.data?.detail||`Failed to submit leave request.`,`error`)}finally{L(!1)}},U=async(e,t)=>{let r=``;if(t===`reject`){if(r=await a(`Please enter comments/reason for rejection:`),r===null)return}else if(t===`return`){if(r=await a(`Please enter comments/remarks for returning this request:`),r===null)return}else if(r=await a(`Enter any comments (optional):`,``),r===null)return;try{await i.post(`/api/leaves/${e}/action/`,{action:t,comments:r}),n(`Leave request successfully updated.`,`success`),H()}catch(e){n(e.response?.data?.detail||`Failed to update leave request.`,`error`)}},W=v.filter(e=>{let t=e.user_full_name||``,n=e.emp_id||``;return t.toLowerCase().includes(R.toLowerCase())||n.toLowerCase().includes(R.toLowerCase())}),G=e=>{w===e?E(T===`asc`?`desc`:`asc`):(fe(e),E(`asc`))},K=[...W].sort((e,t)=>{let n=e[w]||``,r=t[w]||``;return w===`total_days`&&(n=Math.ceil((new Date(e.to_date)-new Date(e.from_date))/(1e3*60*60*24))+1,r=Math.ceil((new Date(t.to_date)-new Date(t.from_date))/(1e3*60*60*24))+1),n<r?T===`asc`?-1:1:n>r?T===`asc`?1:-1:0}),q=Math.ceil(K.length/8),J=D*8,Y=J-8,X=K.slice(Y,J),Z=()=>{let e=`data:text/csv;charset=utf-8,`;e+=`Employee,Employee ID,From Date,To Date,Leave Type,Reason,Status\r
