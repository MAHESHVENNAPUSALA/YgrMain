import{a as e,c as t,f as n,i as r,m as i,p as a,t as o,u as s}from"./index-DGWqTciu.js";var c=i(n(),1),l=a(),u=()=>{let{user:n}=e(),{showToast:i}=r(),{confirm:a}=o(),u=s(),d=n?.role,[f,p]=(0,c.useState)(d===`MD`&&u.pathname===`/holiday-approvals`?`approvals`:`calendar`),[m,h]=(0,c.useState)([]),[g,_]=(0,c.useState)([]),[v,y]=(0,c.useState)(!0),[b,x]=(0,c.useState)(``),[S,C]=(0,c.useState)(``),[w,T]=(0,c.useState)(``),[E,D]=(0,c.useState)(!1),[O,k]=(0,c.useState)(null),A=async()=>{y(!0);try{let e=(await t.get(`/api/holidays/`)).data;h(e.filter(e=>e.status===`Approved`)),_(e.filter(e=>e.status===`Pending`))}catch(e){console.error(`Error loading holidays:`,e)}finally{y(!1)}};(0,c.useEffect)(()=>{A()},[]);let j=async e=>{e.preventDefault(),D(!0);try{await t.post(`/api/holidays/`,{name:b,date:S,department:w}),i(`Holiday created successfully.`,`success`),x(``),C(``),T(``),A()}catch(e){i(e.response?.data?.detail||`Failed to create holiday.`,`error`)}finally{D(!1)}},M=async e=>{if(await a(`Are you sure you want to delete this holiday?`))try{await t.delete(`/api/holidays/${e}/`),i(`Holiday deleted.`,`success`),A()}catch(e){i(e.response?.data?.detail||`Failed to delete holiday.`,`error`)}},N=async(e,n)=>{k(e+n);try{i((await t.post(`/api/holidays/${e}/action/`,{action:n})).data.detail,n===`approve`?`success`:`info`),A()}catch(e){i(e.response?.data?.detail||`Failed to ${n} holiday.`,`error`)}finally{k(null)}},P=e=>({Approved:{background:`#d1fae5`,color:`#065f46`},Pending:{background:`#fef3c7`,color:`#92400e`},Rejected:{background:`#fee2e2`,color:`#991b1b`},Draft:{background:`#e0e7ff`,color:`#3730a3`}})[e]||{background:`#f1f5f9`,color:`#475569`};return(0,l.jsxs)(`div`,{children:[(0,l.jsx)(`style`,{children:`
        .holiday-tabs {
          display: flex;
          gap: 10px;
          margin-bottom: 24px;
