import{a as e,c as t,f as n,i as r,m as i,o as a,p as o,t as ee}from"./index-E7nG5Fmr.js";var s=i(n(),1),c=o(),l=()=>{let{user:n}=e(),{showToast:i}=r(),{confirm:o,prompt:l}=ee(),u=n?.role,[d,f]=(0,s.useState)(`dashboard`),[te,p]=(0,s.useState)(!0),[m,ne]=(0,s.useState)([]),[h,g]=(0,s.useState)(null),[_,re]=(0,s.useState)(null),[v,ie]=(0,s.useState)(null),[y,ae]=(0,s.useState)([]),[b,oe]=(0,s.useState)([]),[se,ce]=(0,s.useState)([]),[x,S]=(0,s.useState)(`info`),[C,le]=(0,s.useState)(``),[w,ue]=(0,s.useState)(``),[T,de]=(0,s.useState)(``),[E,fe]=(0,s.useState)(``),[D,pe]=(0,s.useState)(``),[O,me]=(0,s.useState)(``),[he,k]=(0,s.useState)(!1),[ge,A]=(0,s.useState)(!1),[_e,j]=(0,s.useState)(!1),[ve,M]=(0,s.useState)(!1),[N,P]=(0,s.useState)(null),[F,I]=(0,s.useState)({name:``,description:``,client_name:``,client_contact:``,client:``,project_category:``,priority:`Medium`,start_date:``,end_date:``,estimated_budget:``,technology_stack:``,project_color:`#3b82f6`,assigned_manager:``}),[L,R]=(0,s.useState)(null),[z,B]=(0,s.useState)({name:``,lead:``,department:`python_dev`,description:``,max_size:10}),[V,H]=(0,s.useState)(``),[U,W]=(0,s.useState)(null),[G,K]=(0,s.useState)(``),q=async()=>{try{re((await a.getDashboard()).data)}catch(e){console.error(`Error fetching dashboard data:`,e)}},J=async()=>{try{let e={search:C,status:w,priority:T,manager_id:E,client_name:D,category:O};ne((await a.getProjects(e)).data||[])}catch(e){console.error(`Error fetching projects:`,e),i(`Failed to load projects list.`,`error`)}},Y=async()=>{try{ae((await a.getNotifications()).data||[])}catch(e){console.error(`Error fetching notifications:`,e)}},ye=async()=>{try{oe((await t.get(`/api/users/`)).data||[]),ce((await t.get(`/api/invoicing-resources/`)).data?.clients||[])}catch(e){console.error(`Error fetching users and clients:`,e)}},be=async()=>{if([`HR`,`MD`,`Manager`].includes(u))try{ie((await a.getReports()).data)}catch(e){console.error(`Error fetching reports:`,e)}},X=async()=>{p(!0),await Promise.all([q(),J(),Y(),ye(),be()]),p(!1)};(0,s.useEffect)(()=>{X()},[C,w,T,E,D,O]);let xe=async e=>{try{p(!0),g((await a.getProjectDetail(e.id)).data),S(`info`)}catch(e){console.error(`Error loading project details:`,e),i(`Failed to load project details.`,`error`)}finally{p(!1)}},Z=async()=>{if(h)try{g((await a.getProjectDetail(h.id)).data)}catch(e){console.error(`Error refreshing project details:`,e)}},Q=(0,s.useMemo)(()=>b.filter(e=>e.role===`Manager`),[b]),Se=(0,s.useMemo)(()=>b.filter(e=>e.role===`TeamLead`),[b]),Ce=(0,s.useMemo)(()=>b.filter(e=>e.role===`Employee`),[b]),we=async e=>{if(e.preventDefault(),!F.name){i(`Project Name is required.`,`warning`);return}let t=new FormData;Object.keys(F).forEach(e=>{t.append(e,F[e])}),L&&t.append(`project_logo`,L);try{await a.createProject(t,{headers:{"Content-Type":`multipart/form-data`}}),i(`Project created successfully!`,`success`),k(!1),I({name:``,description:``,client_name:``,client_contact:``,client:``,project_category:``,priority:`Medium`,start_date:``,end_date:``,estimated_budget:``,technology_stack:``,project_color:`#3b82f6`,assigned_manager:``}),R(null),X()}catch(e){i(e.response?.data?.detail||`Failed to create project.`,`error`)}},Te=async e=>{if(e.preventDefault(),!F.name){i(`Project Name is required.`,`warning`);return}let t=new FormData;Object.keys(F).forEach(e=>{t.append(e,F[e])}),L&&t.append(`project_logo`,L);try{await a.updateProject(h.id,t,{headers:{"Content-Type":`multipart/form-data`}}),i(`Project details updated successfully!`,`success`),A(!1),R(null),await Z(),await J(),await q()}catch(e){i(e.response?.data?.detail||`Failed to update project.`,`error`)}},Ee=()=>{h&&(I({name:h.name,description:h.description||``,client_name:h.client_name||``,client_contact:h.client_contact||``,client:h.client||``,project_category:h.project_category||``,priority:h.priority||`Medium`,start_date:h.startdate||``,end_date:h.deadline||``,estimated_budget:h.estimated_budget||``,technology_stack:h.technology_stack||``,project_color:h.project_color||`#3b82f6`,assigned_manager:h.assigned_manager||``,status:h.status||`Pending`}),A(!0))},De=async()=>{if(!h)return;let e=h.is_archived?`unarchive`:`archive`;if(await o(`Are you sure you want to ${e} this project?`))try{let t=await a.archiveProject(h.id,!h.is_archived);i(`Project has been ${e}d successfully.`,`success`),g(e=>e?{...e,is_archived:t.data.is_archived,status:t.data.is_archived?`Archived`:`Active`}:null),J(),q()}catch(e){i(e.response?.data?.detail||`Failed to toggle archive status.`,`error`)}},Oe=async()=>{if(!h)return;let e=allUsers.filter(e=>e.role===`Manager`);if(e.length===0){i(`No managers available in the system.`,`warning`);return}let t=await l(`Transfer project to a new Manager. Available Managers:\n${e.map(e=>`ID: ${e.id} - ${e.name}`).join(`
`)}\n\nEnter the Manager's ID:`);if(t===null||t.trim()===``)return;let n=parseInt(t.trim());if(isNaN(n)){i(`Please enter a valid numeric ID.`,`warning`);return}try{await a.transferProject(h.id,n),i(`Project transferred successfully.`,`success`),await Z(),await J()}catch(e){i(e.response?.data?.detail||`Failed to transfer project.`,`error`)}},$=async e=>{try{await t.post(`/api/projects/${h.id}/review/`,{action:e,remarks:`Reviewed from workspace`}),i(`Project review processed successfully.`,`success`),loadData(),g(null)}catch(e){i(e.response?.data?.detail||`Failed to process project review.`,`error`)}},ke=async()=>{if(h&&await o(`WARNING: Are you sure you want to permanently delete this project? This will remove all teams, members, comments, and files!`))try{await a.deleteProject(h.id),i(`Project deleted successfully.`,`success`),g(null),X()}catch{i(`Failed to delete project.`,`error`)}},Ae=async e=>{if(e.preventDefault(),!z.name){i(`Team Name is required.`,`warning`);return}try{await a.createTeam(h.id,z),i(`Team created successfully!`,`success`),j(!1),B({name:``,lead:``,department:`python_dev`,description:``,max_size:10}),await Z(),await q()}catch(e){i(e.response?.data?.detail||`Failed to create team.`,`error`)}},je=async e=>{if(await o(`Are you sure you want to delete this team?`))try{await a.deleteTeam(e),i(`Team deleted successfully.`,`success`),await Z(),await q()}catch{i(`Failed to delete team.`,`error`)}},Me=async e=>{let t=Se.map(e=>`${e.id}: ${e.name||e.username}`).join(`
`),n=await l(`Assign Team Leader to '${e.name}'. Available Leads:\n${t}\n\nEnter Lead User ID (or leave blank to remove):`);if(n===null)return;let r=n.trim()===``?null:parseInt(n.trim());if(r!==null&&isNaN(r)){i(`Please enter a valid numeric ID.`,`warning`);return}try{await a.updateTeam(e.id,{lead:r}),i(`Team Leader updated successfully.`,`success`),await Z()}catch(e){i(e.response?.data?.detail||`Failed to update Team Leader.`,`error`)}},Ne=async e=>{if(N)try{await a.addTeamMember(N.id,e),i(`Employee assigned to team successfully.`,`success`),M(!1),P(null),await Z(),await q()}catch(e){i(e.response?.data?.detail||`Failed to assign employee.`,`error`)}},Pe=async(e,t,n)=>{if(await o(`Are you sure you want to remove ${n} from this team?`))try{await a.removeTeamMember(e,t),i(`Employee removed from team.`,`success`),await Z(),await q()}catch{i(`Failed to remove employee.`,`error`)}},Fe=async e=>{if(e.preventDefault(),V.trim())try{await a.addComment(h.id,V),H(``),await Z()}catch{i(`Failed to post comment.`,`error`)}},Ie=async e=>{if(e.preventDefault(),!U){i(`Please select a file to upload.`,`warning`);return}let t=new FormData;t.append(`file`,U),t.append(`name`,G||U.name);try{await a.uploadDocument(h.id,t,{headers:{"Content-Type":`multipart/form-data`}}),i(`Document uploaded successfully.`,`success`),W(null),K(``),await Z()}catch(e){i(e.response?.data?.detail||`Failed to upload document.`,`error`)}},Le=async e=>{try{i(`Generating ${e.toUpperCase()} report...`,`info`);let n=await t.get(`/api/projects/reports/`,{params:{export:e},responseType:`blob`}),r=new Blob([n.data],{type:e===`pdf`?`application/pdf`:`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`}),a=URL.createObjectURL(r),o=document.createElement(`a`);o.href=a,o.setAttribute(`download`,`Enterprise_Project_Management_Report.${e===`pdf`?`pdf`:`xlsx`}`),document.body.appendChild(o),o.click(),o.parentNode.removeChild(o),i(`Report downloaded successfully.`,`success`)}catch(e){console.error(e),i(`Failed to export report.`,`error`)}},Re=async(e,t=!1)=>{try{let n=t?{mark_all:!0}:{id:e};await a.markNotificationRead(n),await Y(),t&&i(`All notifications marked as read.`,`success`)}catch(e){console.error(e)}};return te&&!h&&m.length===0?(0,c.jsxs)(`div`,{style:{display:`flex`,justifyContent:`center`,alignItems:`center`,minHeight:`400px`,color:`var(--muted)`},children:[(0,c.jsx)(`i`,{className:`fa-solid fa-spinner fa-spin`,style:{fontSize:`2rem`,marginRight:`10px`}}),` Loading Enterprise Projects Module...`]}):(0,c.jsxs)(`div`,{className:`projects-workspace-container`,children:[(0,c.jsx)(`style`,{children:`
