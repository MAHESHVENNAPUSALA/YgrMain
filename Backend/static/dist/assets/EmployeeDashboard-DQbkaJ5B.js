import{d as e,f as t,o as n,r,s as i,u as a}from"./index-OYoV3RNy.js";var o=t(a(),1),s=e(),c=()=>{let{user:e}=r(),[t,a]=(0,o.useState)(null),[c,l]=(0,o.useState)(null),[u,d]=(0,o.useState)(null),[f,p]=(0,o.useState)([]),[m,h]=(0,o.useState)(!0),[g,_]=(0,o.useState)(`Good day`),[v,y]=(0,o.useState)(``),[b,x]=(0,o.useState)(!1),[S,C]=(0,o.useState)({});(0,o.useEffect)(()=>{(async()=>{try{let e=new Date,t=e.getMonth()+1,r=e.getFullYear(),i=async e=>{try{return await n.get(e)}catch(t){return console.warn(`Failed to fetch ${e}:`,t),{data:null}}},[o,s,c,u]=await Promise.all([i(`/api/dashboard/employee/`),i(`/api/attendance/monthly/?month=${t}&year=${r}`),i(`/api/leaves/`),i(`/api/holidays/`)]);o.data&&a(o.data),s.data&&l(s.data),c.data&&d(c.data),u.data&&p(u.data)}catch(e){console.error(`Error loading employee dashboard metrics:`,e)}finally{h(!1)}})()},[]),(0,o.useEffect)(()=>{let e=()=>{let e=new Date;y(e.toLocaleTimeString(`en-US`,{hour:`2-digit`,minute:`2-digit`,second:`2-digit`,hour12:!0}));let t=e.getHours();_(t>=5&&t<12?`Good morning`:t>=12&&t<17?`Good afternoon`:`Good evening`)};e();let t=setInterval(e,1e3);return()=>clearInterval(t)},[]);let w=e=>{C(t=>({...t,[e]:!t[e]}))},T=e=>{if(!e)return`??`;let t=e.trim().split(/\s+/);return t.length>=2?(t[0][0]+t[t.length-1][0]).toUpperCase():t[0].substring(0,2).toUpperCase()},E=e=>{let t=[`linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%)`,`linear-gradient(135deg, #10b981 0%, #047857 100%)`,`linear-gradient(135deg, #6366f1 0%, #4338ca 100%)`,`linear-gradient(135deg, #8b5cf6 0%, #6d28d9 100%)`,`linear-gradient(135deg, #ec4899 0%, #be185d 100%)`,`linear-gradient(135deg, #f59e0b 0%, #b45309 100%)`,`linear-gradient(135deg, #14b8a6 0%, #0f766e 100%)`],n=0;if(e)for(let t=0;t<e.length;t++)n=e.charCodeAt(t)+((n<<5)-n);return t[Math.abs(n)%t.length]};if(m)return(0,s.jsxs)(`div`,{style:{display:`flex`,justifyContent:`center`,alignItems:`center`,minHeight:`300px`,color:`var(--muted)`},children:[(0,s.jsx)(`i`,{className:`fa-solid fa-spinner fa-spin`,style:{fontSize:`2rem`,marginRight:`10px`}}),` Loading dashboard...`]});let D=t?.tasks?.length||0,O=t?.tasks?.filter(e=>e.status===`Pending`).length||0,k=t?.tasks?.filter(e=>e.status===`In Progress`).length||0,A=t?.tasks?.filter(e=>e.status===`Completed`).length||0,j=t?.tasks?.filter(e=>e.status===`Completed`?!1:new Date(e.end_date)<new Date).length||0,M=t?.tasks?.filter(e=>e.priority===`High`).length||0,N=new Date().toISOString().split(`T`)[0];c?.days_data?.find(e=>e.date===N);let P=c?.stats?.percentage||100,F=D>0?Math.round(A/D*100):100,I=new Date;I.setHours(0,0,0,0);let L=u?.leaves?.some(e=>{if(e.status!==`Final Approved`)return!1;let t=new Date(e.from_date);t.setHours(0,0,0,0);let n=new Date(e.to_date);return n.setHours(0,0,0,0),I>=t&&I<=n});return(0,s.jsxs)(`div`,{className:`employee-dashboard-content`,children:[(0,s.jsx)(`style`,{children:`
