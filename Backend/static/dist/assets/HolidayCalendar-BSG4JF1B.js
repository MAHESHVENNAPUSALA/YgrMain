import{c as e,d as t,f as n,n as r,o as i,r as a,t as o,u as s}from"./index-OYoV3RNy.js";var c=n(s(),1),l=t(),u=()=>{let{user:t}=a(),{showToast:n}=r(),{confirm:s}=o(),u=e(),d=t?.role,[f,p]=(0,c.useState)(d===`MD`&&u.pathname===`/holiday-approvals`?`approvals`:`calendar`),[m,h]=(0,c.useState)([]),[g,_]=(0,c.useState)([]),[v,y]=(0,c.useState)(!0),[b,x]=(0,c.useState)(``),[S,C]=(0,c.useState)(``),[w,T]=(0,c.useState)(``),[E,D]=(0,c.useState)(!1),[O,k]=(0,c.useState)(null),A=async()=>{y(!0);try{let e=(await i.get(`/api/holidays/`)).data;h(e.filter(e=>e.status===`Approved`)),_(e.filter(e=>e.status===`Pending`))}catch(e){console.error(`Error loading holidays:`,e)}finally{y(!1)}};(0,c.useEffect)(()=>{A()},[]);let j=async e=>{e.preventDefault(),D(!0);try{await i.post(`/api/holidays/`,{name:b,date:S,department:w}),n(`Holiday created successfully.`,`success`),x(``),C(``),T(``),A()}catch(e){n(e.response?.data?.detail||`Failed to create holiday.`,`error`)}finally{D(!1)}},M=async e=>{if(await s(`Are you sure you want to delete this holiday?`))try{await i.delete(`/api/holidays/${e}/`),n(`Holiday deleted.`,`success`),A()}catch(e){n(e.response?.data?.detail||`Failed to delete holiday.`,`error`)}},N=async(e,t)=>{k(e+t);try{n((await i.post(`/api/holidays/${e}/action/`,{action:t})).data.detail,t===`approve`?`success`:`info`),A()}catch(e){n(e.response?.data?.detail||`Failed to ${t} holiday.`,`error`)}finally{k(null)}},P=e=>({Approved:{background:`#d1fae5`,color:`#065f46`},Pending:{background:`#fef3c7`,color:`#92400e`},Rejected:{background:`#fee2e2`,color:`#991b1b`},Draft:{background:`#e0e7ff`,color:`#3730a3`}})[e]||{background:`#f1f5f9`,color:`#475569`};return(0,l.jsxs)(`div`,{children:[(0,l.jsx)(`style`,{children:`
        .holiday-tabs {
          display: flex;
          gap: 10px;
          margin-bottom: 24px;
