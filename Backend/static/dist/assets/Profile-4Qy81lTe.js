import{c as e,d as t,f as n,i as r,l as i,m as a,p as o,u as s}from"./index-E7nG5Fmr.js";var c=a(n(),1),l=o(),u=()=>{let{showToast:n}=r(),[a,o]=(0,c.useState)(null),[u,d]=(0,c.useState)(!0),[f,p]=(0,c.useState)(()=>{let e=new Date;return`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,`0`)}`}),m=(0,c.useRef)(null),h=s(),g=t(),_=h.pathname===`/profile/edit`,[v,y]=(0,c.useState)(!1),b=v||_,[x,S]=(0,c.useState)(``),[C,w]=(0,c.useState)(``),[T,E]=(0,c.useState)(``),[D,O]=(0,c.useState)(``),[k,A]=(0,c.useState)(``),[j,M]=(0,c.useState)(`Male`),[N,P]=(0,c.useState)(null),[F,I]=(0,c.useState)(!1),[L,R]=(0,c.useState)(!1);(0,c.useEffect)(()=>{if(a&&a.user){let e=a.user;S(e.first_name||``),w(e.email||``),E(e.phone||``),O(e.address||``),A(e.date_of_birth||``),M(e.gender||`Male`)}},[a]);let z=async t=>{t.preventDefault(),R(!0);try{let t=new FormData;t.append(`first_name`,x),t.append(`email`,C),t.append(`phone`,T),t.append(`address`,D),t.append(`date_of_birth`,k),t.append(`gender`,j),N&&t.append(`profile_pic`,N),F&&t.append(`remove_profile_pic`,`true`),await e.put(`/api/profile/`,t,{headers:{"Content-Type":`multipart/form-data`}}),n(`Profile updated successfully!`,`success`),y(!1),_&&g(`/profile`),B(f)}catch(e){n(e.response?.data?.detail||`Failed to update profile.`,`error`)}finally{R(!1)}},B=async t=>{d(!0);try{let n=t?{month:t}:{},r=await e.get(`/api/profile/`,{params:n});console.log(`Profile API Complete Response:`,{url:`/api/profile/`,method:`GET`,headers:r.config?.headers||{},status:r.status,payload:r.data}),o(r.data)}catch(e){console.error(`Failed to load profile:`,e)}finally{d(!1)}};(0,c.useEffect)(()=>{B(f)},[]),(0,c.useEffect)(()=>{a&&m.current&&setTimeout(()=>{m.current&&(m.current.style.width=`${a.attendance?.percentage||0}%`)},300)},[a]);let V=e=>{e.preventDefault(),B(f)},H=e=>{if(!e||e===`None`)return`—`;let t=new Date(e),n=((new Date-t)/(1e3*60*60*24*365.25)).toFixed(1);return isNaN(n)?`—`:n},U=e=>e?new Date(e).toLocaleDateString(`en-GB`,{day:`2-digit`,month:`short`,year:`numeric`}):`—`,W=e=>e===`Completed`?`completed`:e===`In Progress`?`inprogress`:`pending`;if(u)return(0,l.jsx)(`div`,{style:{display:`flex`,alignItems:`center`,justifyContent:`center`,height:`60vh`},children:(0,l.jsx)(`i`,{className:`fa-solid fa-spinner fa-spin`,style:{fontSize:32,color:`#3b82f6`}})});let{user:G,attendance:K,leave_summary:q,projects:J,salary_structure:Y}=a||{},X=(e.defaults.baseURL||`http://127.0.0.1:8000`).replace(/\/$/,``),Z=G?.profile_pic_url||(G?.profile_pic?G.profile_pic.startsWith(`http`)?G.profile_pic:`${X}${G.profile_pic}`:null),Q=G?.first_name&&(G.first_name+` `+(G.last_name||``)).trim()||G?.username||`—`,$=G?.department_display||G?.department||`—`;return(0,l.jsxs)(`div`,{className:`profile-page`,children:[(0,l.jsx)(`style`,{children:`
