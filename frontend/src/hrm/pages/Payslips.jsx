...
const Payslips = () => {
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      showToast('Payslip generated and published successfully.', 'success');
      setActiveTab('list');
      loadPayslips();
    } catch (err) {
      showToast(err.response?.data?.detail || 'Failed to generate payslip.', 'error');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    const isConfirmed = await showConfirm('Are you sure you want to delete this payslip record?');
    ...
    try {
      ...
      showToast(err.response?.data?.detail || 'Failed to delete payslip.', 'error');
    }
  };
                          ...
                          <tr key={p.id}>
                            ...
                            <td>
                              <div style={{ display: 'flex', gap: '6px' }}>
                                ...
                                {p.payslip_pdf && (
                                  <a href={p.payslip_pdf.startsWith('http') ? p.payslip_pdf : `${(import.meta.env.VITE_API_URL || api.defaults.baseURL || 'http://127.0.0.1:8000').replace(/\/$/, '')}${p.payslip_pdf}`} download className="download-btn">
                                    ⬇ PDF
                                  </a>
                                )}
                                {['HR', 'MD'].includes(role) && (
          ...
          <div className="modal-container" style={{ maxWidth: '600px' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ background: '#f8fafc', padding: '12px', borderRadius: '8px', border: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {selectedSlipDetail.payslip_pdf && (
                <a href={selectedSlipDetail.payslip_pdf.startsWith('http') ? selectedSlipDetail.payslip_pdf : `${(api.defaults.baseURL || 'http://127.0.0.1:8000').replace(/\/$/, '')}${selectedSlipDetail.payslip_pdf}`} download className="btn" style={{ marginTop: 0 }}>
                  Download PDF Slip
                </a>
              )}
            </div>
