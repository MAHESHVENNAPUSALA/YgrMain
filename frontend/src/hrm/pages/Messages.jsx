...
const Messages = () => {
  ...
  // ── Unread tracking ──
  // lastReadIds: { [roomId]: lastSeenMsgId } for channels/teams
  const [lastReadIds, setLastReadIds] = useState(() => {
    try { return JSON.parse(localStorage.getItem('chat_last_read') || '{}'); }
    catch { return {}; }
  });
  const lastReadIdsRef = useRef(lastReadIds);
  ...
  const [activeCallSession, setActiveCallSession] = useState(null); // { type, user, status }
  ...
  // ── Update lastReadIds ref when state changes ──
  useEffect(() => {
    lastReadIdsRef.current = lastReadIds;
    localStorage.setItem('chat_last_read', JSON.stringify(lastReadIds));
  }, [lastReadIds]);
  ...
  const loadSidebar = useCallback(async () => {
    try {
      const [usersRes, roomsRes] = await Promise.all([
        chatApi.getAllUsers(),
        chatApi.getAllChatRooms(lastReadIdsRef.current),
      ]);
      const usersData = usersRes.data || [];
      setAllUsers(usersData);
      const chData = roomsRes.data?.channels || [];
      const tmData = roomsRes.data?.teams || [];
      setChannels(chData);
      setTeams(tmData);

      // Fire total unread count update for sidebar badge
      const dmUnread = usersData.reduce((s, u) => s + (u.unread || 0), 0);
      const roomUnread = [...chData, ...tmData].reduce((s, r) => s + (r.unread || 0), 0);
      window.dispatchEvent(new CustomEvent('msg-unread-update', { detail: dmUnread + roomUnread }));
    } catch (e) {
  ...
  const handleCall = async (callType) => {
    ...
    try {
      ...
      // Simulate connected state after 2.5 seconds
      setTimeout(() => {
      ...
      showToast(err.response?.data?.detail || 'Failed to initiate call session.', 'error');
  ...
  useEffect(() => {
    ...
    // Poll sidebar for updated unread counts every 10 seconds
    const sidebarPoll = setInterval(() => loadSidebar(), 10000);
    return () => {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      showToast(e?.response?.data?.detail || 'Failed to send message.', 'error');
    }
  };
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      showToast(e?.response?.data?.detail || 'Edit failed.', 'error');
    }
    setEditingMsg(null);
    setEditText('');
  };
  ...
  const handleCreateTeam = async (e) => {
    ...
    try {
      ...
      loadSidebar();
    } catch (e) { showToast(e?.response?.data?.detail || 'Failed to create team.', 'error'); }
  };
  ...
  return (
    <div>
          ...
          <div className="uc-sidebar-list">
            ...
            {loadingSidebar ? (
              ...
              <div
                ...
                onClick={() => {
                  setActiveChat({ type: 'channel', id: ch.id, name: ch.name, is_announcement_only: ch.is_announcement_only });
                  if (ch.last_msg_id) {
                    setLastReadIds(prev => ({ ...prev, [ch.id]: ch.last_msg_id }));
              ...
              >
                ...
                {ch.unread > 0 && (
                  <div className="uc-unread">{ch.unread > 99 ? '99+' : ch.unread}</div>
                )}
              </div>
            ))}
              ...
              <>
                ...
                {filteredTeams.map(t => (
                  <div
                    ...
                    onClick={() => {
                      setActiveChat({ type: 'team', id: t.id, name: t.name });
                      if (t.last_msg_id) {
                        setLastReadIds(prev => ({ ...prev, [t.id]: t.last_msg_id }));
                        setTeams(prev => prev.map(tm => tm.id === t.id ? { ...tm, unread: 0 } : tm));
                      }
                    }}
                  >
                    ...
                    {t.unread > 0 && (
                      <div className="uc-unread">{t.unread > 99 ? '99+' : t.unread}</div>
                    )}
                  </div>
                ))}
              </>
            ...
            {filteredUsers.map(u => (
              <div
                ...
                onClick={() => {
                  setActiveChat({ type: 'dm', id: u.id, name: u.name });
                  // Optimistically clear DM unread count
                  setAllUsers(prev => prev.map(usr => usr.id === u.id ? { ...usr, unread: 0 } : usr));
                }}
              >
            ...
            <>
              ...
              <div className="uc-msgs">
                {loadingMsgs ? (
                  ...
                  const isGroup = activeChat.type !== 'dm';
                  ...
                  return (
                    ...
                    >
                      {/* Avatar — always shown beside bubble */}
                      {!isSent ? (
                        <Avatar user={{ sender_avatar: msg.sender_avatar, name: msg.sender_name }} size={30} style={{ flexShrink: 0 }} />
                      ) : (
                        <Avatar user={{ sender_avatar: user?.profile_pic, name: user?.first_name || user?.username }} size={30} style={{ flexShrink: 0 }} />
                      )}
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: isSent ? 'flex-end' : 'flex-start', maxWidth: '65%' }}>
                        ...
                        >
                          ...
                          {isDeleted ? (
                            ...
                            <>
                              {/* Text — render paragraphs for multi-line messages */}
                              {msg.text && (
                ...
                <div className="uc-input-wrap" style={{ position: 'relative' }}>
                  ...
                  <form onSubmit={handleSend}>
                    <div className="uc-input-row">
                      ...
                      {/* Text input — textarea for multi-line paragraphs */}
                      <textarea
                        ref={inputRef}
                        className="uc-input-box"
                        rows={1}
                        placeholder={`Message ${activeChat.type === 'dm' ? activeChatTitle : '#' + activeChatTitle}… (Shift+Enter for new line)`}
                        value={inputText}
      ...
      {/* ─── CALL RINGING MODAL ───────────────────────── */}
      {activeCallSession && (
        <div className="uc-modal-overlay" style={{ zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(15, 23, 42, 0.75)' }}>
          <div className="uc-modal" style={{ background: '#0f172a', color: '#fff', textAlign: 'center', padding: '40px 30px', maxWidth: '380px', borderRadius: '16px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.5)' }}>
            <div style={{ position: 'relative', display: 'inline-block', marginBottom: '24px' }}>
              <div className="uc-call-avatar-pulse"></div>
              <Avatar user={activeCallSession.user} size={100} style={{ border: '4px solid #3b82f6', position: 'relative', zIndex: 2 }} />
            </div>
            
            <h3 style={{ fontSize: '20px', fontWeight: 800, marginBottom: '6px', color: '#fff' }}>{activeCallSession.user.name}</h3>
            <p style={{ fontSize: '13px', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '30px' }}>{activeCallSession.user.role || 'Employee'}</p>
            
            <div style={{ fontSize: '16px', fontWeight: 600, color: '#3b82f6', marginBottom: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
              <span className="uc-call-dot-pulse"></span>
            ...
            <div style={{ display: 'flex', gap: '20px', justifyContent: 'center' }}>
              <button 
                ...
                title="Hang up"
              >
                <i className="fa-solid fa-phone-slash" style={{ transform: 'rotate(-45deg)' }}></i>
              </button>
            </div>
          </div>
        </div>
